UC07_login()
{
	...
	lr_start_transaction("UC07_TR02_login");

	web_reg_find("Text=logout","SaveCount=count",LAST);

	web_submit_data("UC07_TR02_login", 
	...
	{
		lr_end_transaction("UC07_TR02_login", LR_FAIL);
		lr_error_message("Пользователь не авторизован");
		lr_exit(LR_EXIT_ITERATION_AND_CONTINUE, LR_FAIL);
		return 0;
	}
	
	lr_end_transaction("UC07_TR02_login", LR_AUTO);
